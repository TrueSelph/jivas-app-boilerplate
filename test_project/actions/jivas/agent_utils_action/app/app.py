...
def render(router: StreamlitRouter, agent_id: str, action_id: str, info:dict):
    ...
    with st.expander('Import Agent', False):
        #toggle to use YAML default is JSON
        use_yaml = st.toggle("Import Agent YAML")

        if use_yaml:
            agent_yaml = st.text_area('Agent Descriptor in YAML', value="", height=170, key=f"{model_key}_agent_yaml")

            if st.button('Import', key=f"{model_key}_btn_import_agent_yaml"):
                # Call the function to import
                if result := call_import_agent(agent_yaml=agent_yaml):
            ...
            if uploaded_file is not None:
                ...
                if loaded_config:
                    ...
                    if imported:= call_import_agent(agent_data= loaded_config):
                        st.success("Agent imported successfully")
                    else:
            ...
            agent_data = st.text_area('Agent Descriptor in JSON', value={}, height=170, key=f"{model_key}_agent_data")
            agent_data = json.loads(agent_data)
            
            if st.button('Import', key=f"{model_key}_btn_import_agent_json"):
                # Call the function to import
                if result := call_import_agent(agent_data= agent_data):
                    st.success("Agent imported successfully")
                else:
                    st.error("Failed to import agent. Ensure that the  descriptor is in valid JSON format")

            uploaded_file = st.file_uploader("Upload JSON file", key=f"{model_key}_agent_data_upload")
            if uploaded_file is not None:
                ...
                if loaded_config:
                    ...
                    if imported:= call_import_agent(agent_data= loaded_config):
                        ...
                        st.error("Failed to import agent. Ensure that you are uploading a valid JSON file")

                else:
                    st.error("File is invalid. Please upload a valid JSON file")
